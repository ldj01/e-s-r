...
Lut_t *GetLut(int nband, Input_meta_t *meta, Input_meta_t *b6_meta, 
  ...
  this->min_valid_sr = (MIN_VALID_SR - get_offset_refl()) * ( 1 / get_scale_refl());
  if (this->min_valid_sr < 0)
    this->min_valid_sr = 0;
  this->max_valid_sr = (MAX_VALID_SR - get_offset_refl()) * ( 1 / get_scale_refl());
  ...
  this->mult_factor=      1 / get_scale_refl();     /* scale factor            */
